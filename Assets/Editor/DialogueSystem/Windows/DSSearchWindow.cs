...
public class DSSearchWindow : ScriptableObject, ISearchWindowProvider {
	...
	public List<SearchTreeEntry> CreateSearchTree(SearchWindowContext context) {
		List<SearchTreeEntry> searchTreeEntries = new List<SearchTreeEntry>() {
			...
			new SearchTreeEntry(new GUIContent("Root", indentationIcon)) {
				userData = DSDialogueType.Root,
				level = 2
			},
	...
	public bool OnSelectEntry(SearchTreeEntry SearchTreeEntry, SearchWindowContext context) {
		...
		switch (SearchTreeEntry.userData) {
			case DSDialogueType.Root: {
				DSRootNode rootNode = (DSRootNode) graphView.CreateNode("DialogueName", DSDialogueType.Root, localMousePosition);
